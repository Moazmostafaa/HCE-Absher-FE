...
{
    public class DeleteMeasuringUnitCommand : CommandBase<ResponseResult<bool>>
    {
        public Guid Id { get; set; }
        private class Handler : IRequestHandler<DeleteMeasuringUnitCommand, ResponseResult<bool>>
        {
            ...
            public async Task<ResponseResult<bool>> Handle(DeleteMeasuringUnitCommand request, CancellationToken cancellationToken)
            {
                var measuringUnit = await _read.GetAsync(x => x.Id == request.Id);
                if (measuringUnit == null)
                    throw new EntityNotFoundException(Message_Resource.NotFound);

                measuringUnit.IsDeleted = true;
                measuringUnit.DeletedDate = DateTime.Now.GetCurrentDateTime();
            ...
            public class Validator : AbstractValidator<DeleteMeasuringUnitCommand>
            {
                public Validator()
                {
